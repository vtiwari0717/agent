...
class RecommendationEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            # This path MUST match where you put the CSV file in Cursor
            self.nutrition_agent = NutritionAgent(csv_path="data/expanded_food_dataset_10000.csv")
            ...
            # Re-raise the exception so the app knows it failed to start
            raise
    ...
    def generate_plan(self, user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # ---------------------------------------------------------
            # Calculate BMI, BMR, TDEE, and exact Macro targets
            ...
            target_calories = health_analysis["targets"]["calories"]
            diet_pref = user_profile.get("diet_preference", "Veg")

            # ---------------------------------------------------------
            # STEP 2: Generate 7-Day Meal Plan (The Chef)
            # ---------------------------------------------------------
            # We assume the user wants variety, so we generate 7 distinct days
            weekly_plan = {}
            
            for day in range(1, 8):
                day_label = f"Day {day}"
                
                # Get a daily plan from Nutrition Agent
                ...
                # Structure it nicely for the frontend
                weekly_plan[day_label] = {
                    "meals": daily_meals,
                    "calories": sum(m["total_calories"] for m in daily_meals.values()),
            ...
            # ---------------------------------------------------------
            # We pass the full weekly plan to Gemini to generate a summary
            ai_explanation = self.llm_agent.explain_plan(weekly_plan, user_profile)
            ...
            final_plan = {
                ...
                "nutritional_targets": health_analysis["targets"],
